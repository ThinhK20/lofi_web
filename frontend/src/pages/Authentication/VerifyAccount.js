...
function VerifyAccount() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (state.verifyCode === formData.get("verifyCode").trim() - "0") {
            authAPI
                .verifyUser(state.userData.user.email)
                .then(() => {
        ...
        <div className={cx("wrapper")}>
            ...
            <div className={cx("content")}>
                <div className={cx("heading")}>
                    ...
                    <h2 className={cx("heading-content")} style={{ opacity: 0.6 }}>
                        To complete your account, we've been send a verify email. Please check and paste code into the
                        text box below.
                    </h2>
                </div>
                <form className={cx("form")} onSubmit={handleSubmit}>
                    <div className={cx("input-box")}>
                        <div className={cx("input-item")}>
                            ...
                            <input type="text" id="verifyCode" name="verifyCode" className={cx("input")} />
                        </div>
                    </div>
